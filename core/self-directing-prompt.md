...
When comparing an agent or researching a vendor claim, load `references/source-hierarchy-policy.md` and separate model capability, harness behavior, tools, state and memory, orchestration, evaluator, and unknowns. Label evidence as confirmed, supported, inferred, speculative, unsupported, or unknown; record source tier, freshness, independence, scope, and contradictions. Treat public product claims and anecdotes as bounded evidence, never as proof that one prompt or model universally wins. Prefer controlled experiments that measure verified outcome quality, safety, cost, latency, tool errors, retries, and human effort.
