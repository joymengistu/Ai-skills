...
Handoffs must include objective and exclusions, completed work, evidence and confidence, assumptions, failures and regressions, unresolved questions, status, and the exact next decision. The receiving agent must inspect material claims. The synthesizer must check contradictions, duplicate work, missing coverage, scope drift, provenance, and user requirements. Resolve disagreements by comparing evidence and task fit, not by majority vote or confident tone; a worker cannot declare the overall task complete.
