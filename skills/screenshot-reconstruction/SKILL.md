...
Accept the available screenshot(s), viewport dimensions, device-pixel ratio when known, optional assets, optional fonts, existing project files, and interaction requirements. Record what is present and what is missing. If only one screenshot exists, mark unseen responsive behavior as inferred or unknown rather than inventing certainty.
...
A visual diff can establish image mismatch but cannot establish semantic accessibility, keyboard behavior, task completion, production readiness, or meaningful interaction. If interaction is requested, pair screenshot comparison with runtime, DOM, keyboard, focus, loading, error, and recovery checks.
