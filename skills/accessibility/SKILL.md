...
Treat accessibility as part of correctness, not a cosmetic enhancement. Use `references/engineering-quality-gates.md` to record accessibility gate status alongside correctness, failure, security, privacy, performance, compatibility, observability, reproducibility, and maintainability. Identify users, modalities, assistive technologies, language needs, device constraints, cognitive load, and failure consequences.
...
For AI interactions, make status, uncertainty, permissions, errors, cancellation, and next actions understandable. Preserve user input and provide recovery. Test with realistic interruptions, slow networks, screen readers or equivalent assistive technology where available, keyboard-only use, zoom/reflow, reduced motion, and varied comprehension. Report automated, manual, and not-assessable checks separately. Do not infer disability or lower user autonomy because the user requests simplicity.
