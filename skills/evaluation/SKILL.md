...
Maintain train/dev/held-out cases, version prompts and tools, run regression tests, inspect failures, and change one variable at a time where possible. Never use an evaluator that rewards unsupported confidence or blindly copies the system's own rationale.
