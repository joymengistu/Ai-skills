...
Use `references/collaboration-handoff-contract.md` when work crosses agents or workers. At the end of each stage, write a compact handoff containing the objective, current status, scope and exclusions, requirement ledger, decisions, assumptions, artifacts, tests, evidence and confidence, failures and regressions, unresolved questions, exact next decision, next step, and stop conditions. Keep it in durable storage outside the model context window and outside a disposable worker.
...
Keep handoffs concise enough to load reliably, but complete enough to prevent loss of goals, constraints, evidence, ownership, conflicts, and test status. A handoff is context, not proof; the receiving worker must inspect material claims before acting.
