...
For research, keep facts, evidence, inferences, hypotheses, and unknowns distinct. For actions affecting external systems, load the relevant safety and approval Skills and stop for required user confirmation. For accessibility or human-facing output, add `accessibility` and the relevant communication or human-value Skill. When the request uses “best,” “maximum,” “top tier,” “perfect,” “go to your limit,” or similar language, load `references/quality-vocabulary.md` and convert the intensifier into a target, scope, constraints, evidence standard, budget, and stopping rule. When a request explicitly pairs Maximum Generation with planning, or an ambitious observable creative outcome has enough dependencies or uncertainty to require a quality-aware plan, add `maximum-generation-planning` before domain execution; it establishes quality-aware milestones, observation needs, replan triggers, and an iteration budget. Then add `maximum-generation` after the relevant domain Skill and before final runtime verification for the observed quality loop. Skip the planning companion for mechanical, small, or already-simple quality tasks. Neither Skill owns permissions or the final completion decision. Never interpret intensifiers as unlimited resources or authority.
