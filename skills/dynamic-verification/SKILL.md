...
For software, build or launch the artifact, inspect runtime errors, exercise the primary journey, test invalid input, empty and loading states, refresh or restart, permissions, responsive layout, accessibility path, and recovery from tool or network failure. For games, test controls, playability, progression, feedback, pause, restart, and win/lose behavior. For documents or media, inspect rendered pages, legibility, structure, and source fidelity.

Record evidence such as URLs, screenshots, logs, traces, outputs, timestamps, and test steps. Separate build health, visual or interaction usability, and intent alignment. A project that compiles but does not respond correctly is not verified. A beautiful screen that omits the requested workflow is not complete.
