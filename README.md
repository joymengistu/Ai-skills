...
The repository contains a self-directing core prompt, an operating contract, a bounded execution loop, an explicit action protocol, sixty-three modular skills, adapters for the user's Joy and CLAI repositories, a human-satisfaction framework, a security and governance layer, evaluation cases, and validation scripts.
...
| Grounded verification | Confined read-only artifact hashing, verifier-supplied evidence, and completion blocking when the expected environment state is missing or mismatched |
| Regression accounting | Per-case gains, regressions, residual failures, unchanged successes, hard-gate failures, and net deltas kept separate from aggregate scores |
| Professional developer-tool UI | Contextual shell, sidebar, panel, button, spacing, density, focus, loading, responsive, and state-completeness patterns informed by public Replit/Vercel references |
| One-shot execution | Transparent self-directing contract for requirement conservation, proportional planning, thin complete slices, dynamic verification, repair, and honest completion reporting |
| Efficient Skill expansion | Audit-first self-prompt for adding domain-specific workflows, examples, failure modes, composition rules, and verifiers without generic padding or context waste |
| Screenshot reconstruction | Reference-first HTML/CSS/JavaScript reconstruction with screenshot forensics, measured geometry, typography/assets audit, same-viewport rendering, objective diffs, and ordered visual repair |
| Screenshot precision composition | Automatic fidelity-first routing across UI Vision, requirement compilation, product completeness, dynamic verification, accessibility, and secondary professional taste; visual and functional acceptance remain separate |
| Agent Max universal entry | One-touch or slash-command entry point that selects minimal Skill bundles, shows route previews, points to catalog paths, and preserves host permissions and verification boundaries |
| Top-tier improvement program | Six-component, 30-mission roadmap covering quality semantics, research, runtime, professional output, human lovability, evaluation, safety, and Skill curation |
| Quality vocabulary | Operational definitions for basic, good, strong, excellent, best, very best, top tier, maximum, perfect, quick, and “go to your limit,” with evidence, budget, and stopping rules |
| Repository audit | Reproducible 63-Skill capability map, structural audit report, preserved master mission, and execution ledger with explicit limits on what structural signals prove |
| Modern prompt architecture | Concise layered contracts for host authority, intent, context, coordination, tools, memory, output, verification, evaluation, and recovery, grounded in public 2026-era patterns |
| Prompt architecture research | Practical 2026 findings, compact model-facing control block, role boundaries, typed handoffs, memory controls, tool contracts, eval methods, and resumable failure recovery |
...
Run the structural checks from the repository root. The current risk-review milestone adds `references/agent-risk-control-blueprint.md`, `references/agent-risk-research-notes.md`, `runtime/risk-control.schema.json`, and `skills/agent-risk-controls/SKILL.md`. The current UI/lovability milestone adds `references/professional-taste-research-notes.md`, `references/professional-taste-architecture.md`, `skills/professional-taste/SKILL.md`, and `evals/lovability-benchmark-plan.md`. The current sequential-mission milestone adds `references/contextual-user-intelligence-architecture.md`, `references/skill-engineering-intelligence.md`, `references/master-mission-implementation-map.md`, three preserved mission sources, and contextual evaluation cases. The intelligence-infrastructure milestone adds `references/intelligence-infrastructure-audit.md`, public research notes, nine runtime intelligence schemas, representative records, a deterministic kernel and tests, and one focused meta-capability Skill. The hosted-evaluation research milestone adds `references/hosted-evaluation-research-brief-2026-08-23.md` and `references/hosted-evaluation-architecture-research-2026-08-23.md`; it defines the next experiment without claiming that a provider or model is universally better. The provider-adapter milestone adds `runtime/normalized-trace.schema.json`, `runtime/reference_host/normalized_trace.py`, and `references/provider-adapter-boundary.md`; it proves only deterministic normalization and redaction behavior in the reference host, not cross-provider equivalence or production privacy compliance. The maximum-capability increment adds `runtime/reference_host/verifiers.py` for confined read-only artifact verification and per-case paired aggregation in `runtime/intelligence/kernel.py`; it improves evidence quality without claiming a model-performance gain. The professional UI milestone adds `references/professional-ui-patterns.md`, public Replit/Vercel/Geist observations, and four focused UI benchmark cases for shell geometry, icon controls, loading/destructive states, and protected reference boundaries. The one-shot milestone adds `references/one-shot-execution-prompt.md`, integrates it into Ultra Plan and the core self-directing route, and adds five cases for requirement preservation, proportional planning, complete slices, verification, and private-reasoning boundaries. The Skill-expansion milestone adds `references/skill-expansion-audit-2026-08-23.md` and `references/skill-expansion-self-prompt.md`, integrates the protocol into intelligence infrastructure, and adds five anti-padding and promotion-discipline cases. The screenshot-reconstruction milestone adds `skills/screenshot-reconstruction/SKILL.md`, `references/screenshot-reconstruction-architecture.md`, the preserved mission source, and five cases for forensic measurement, visual-diff iteration, responsive uncertainty, and reference fidelity. The screenshot-precision integration adds explicit handoffs in UI Vision, Professional Taste, Requirement Compiler, Product Completeness, Dynamic Verification, and the umbrella router, plus three composition cases; it keeps screenshot fidelity separate from hidden behavior, accessibility, and unseen responsive states. The Agent Max milestone adds `skills/agent-max/SKILL.md` as the universal single-entry Skill, with one-touch `Agent Max` and optional `/agent-max` invocation, stable presets for catalog, screenshot precision, product builds, one-shot builds, research, verification, and Skill discovery, route previews, and two command-routing evaluation cases. It does not duplicate the 62 domain Skills or turn command IDs into permission-granting APIs; the host must provide the actual button or slash-command binding. The 30-mission top-tier roadmap adds `references/30-mission-top-tier-roadmap.md`, a six-component improvement program and calibrated quality vocabulary that translates “good,” “best,” “maximum,” “perfect,” and related intensifiers into evidence, budgets, scope, and stopping rules without implying unlimited resources or authority. Mission 1 adds `scripts/audit_skill_catalog.py`, `references/skill-catalog-capability-map.json`, `references/skill-catalog-audit-2026-08-23.md`, `references/30-mission-execution-ledger.yaml`, and the preserved master mission source. Mission 2 adds `references/quality-vocabulary.md` and routes it from the core prompt and Agent Max. The modern prompt-architecture milestone rewrites `core/layered-system-prompts.md` into a concise implementation-ready stack, records public OpenAI/Anthropic/LangGraph/tool/memory findings, adds a compact model-facing control block, and adds six cases for layer ownership, coordination, tool boundaries, memory authority, recovery, and conservative promotion. The prompt-architecture research package is `references/modern-prompt-architecture-2026-08-23.md`, with source checkpoints in `references/prompt-architecture-research-2026-08-23.md`; it is practical guidance, not a claim that any provider or prompt universally wins. The executable reference host now enforces trust labeling primitives, action intent records, bound approvals, cancellation, tool validation, destination boundaries, and redacted incident journaling.
